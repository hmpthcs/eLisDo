...
fn read_panel_pwr_file(parameter: &str) -> String {
    let parameter_file = "/sys/devices/platform/fdec0000/power/".to_owned() + &parameter;
    let file = OpenOptions::new()
        .read(true)
        .open(parameter_file)
        .expect("Error opening the file");
...
fn write_panel_pwr_file(parameter: &str, new_value: &str) {
    let device = "/sys/devices/platform/fdec0000/power/".to_owned() + &parameter;
    let file = OpenOptions::new()
        .write(true)
        .open(device)
        .expect("Error opening the file");
...
pub fn set_panel_pwr_control(control: &str) {
    let ebc_device = "/sys/devices/platform/fdec0000/power/control";
    let file = OpenOptions::new()
        .read(true)
        .write(true)
        .open(ebc_device)
    ...
    write!(&file, "{}", control).unwrap();
}

//pub fn get_panel_pwr_control() -> &str {
//    let control_str = read_panel_pwr_file("control").parse::<str>().unwrap();
//    return control_str;
//}
...
pub fn set_refresh_waveform(waveform: u8) {
    let ebc_device = "/sys/module/rockchip_ebc/parameters/refresh_waveform";
    let file = OpenOptions::new()
        .read(true)
        .write(true)
        .open(ebc_device)
...
pub fn get_refresh_waveform() -> u8 {
    let waveform_u8 = read_ebc_file("default_waveform").parse::<u8>().unwrap();
    return waveform_u8;
}
